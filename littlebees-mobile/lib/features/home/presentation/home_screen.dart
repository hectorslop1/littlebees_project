...
class HomeScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return childrenAsync.when(
      loading: () => const Scaffold(body: SafeArea(child: HomeShimmer())),
      error: (error, stack) => Scaffold(
        body: SafeArea(
          child: Center(child: Text('Error loading children: $error')),
        ),
      ),
      data: (children) {
        if (children.isEmpty) {
          return Scaffold(
            body: SafeArea(
              child: Center(child: Text(tr.tr('no_children_found'))),
            ),
          );
        }
        ...
        if (currentChildId == null) {
          ...
          return const Scaffold(body: SafeArea(child: HomeShimmer()));
        }
              ...
              data: (dailyStory) {
                return RefreshIndicator(
                  ...
                  child: CustomScrollView(
                    key: ValueKey(
                      currentChildId,
                    ), // Force rebuild and re-animate on child change
                    slivers: [
                      SliverAppBar(
